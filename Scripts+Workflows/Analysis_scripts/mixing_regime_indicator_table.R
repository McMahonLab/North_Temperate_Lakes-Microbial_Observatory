...
poly.indicators <- poly[c(1, 3, 4, 5, 6, 9, 14, 20, 21, 22), 4:5]
find <- match(rownames(poly.indicators), rownames(full_table))
abundance <- rowSums(full_table[find,])/total
poly.indicators$Abundance <- abundance * 100
poly.indicators$p.value <- NULL
poly.indicators$index <- NULL
colnames(poly.indicators) <- c("Phi", "% of Community")
rownames(poly.indicators) <- gsub("k__Bacteria;|p__|c__|o__|g__|f__|;g__;s__1||\\[|\\]", "", rownames(poly.indicators))
write.csv(poly.indicators, file = "C:/Users/Alex/Desktop/North_Temperate_Lakes-Microbial_Observatory/Data/polymictic_indicators.csv", row.names=T)

dim <- results[which(results$index == 2), ]
dim <- dim[order(dim$stat, decreasing=T), ]

dim.indicators <- dim[c(1, 2, 5, 8, 11, 12, 15, 16, 19, 20), 4:5]
find <- match(rownames(dim.indicators), rownames(full_table))
abundance <- rowSums(full_table[find, ])/total
dim.indicators$Abundance <- abundance * 100
dim.indicators$p.value <- NULL
dim.indicators$index <- NULL
colnames(dim.indicators) <- c("Phi", "% of Community")
rownames(dim.indicators) <- gsub("k__Bacteria;|p__|c__|o__|g__|f__|;g__;s__1||\\[|\\]", "", rownames(dim.indicators))
write.csv(dim.indicators, file = "C:/Users/Alex/Desktop/North_Temperate_Lakes-Microbial_Observatory/Data/dimictic_indicators.csv", row.names = T)

mero <- results[which(results$index == 3), ]
mero <- mero[order(mero$stat, decreasing = T), ]

mero.indicators <- mero[c(1, 6, 7, 9, 11, 12, 14, 15, 19, 21), 4:5]
find <- match(rownames(mero.indicators), rownames(full_table))
abundance <- rowSums(full_table[find,])/total
mero.indicators$Abundance <- abundance * 100
mero.indicators$p.value <- NULL
mero.indicators$index <- NULL
colnames(mero.indicators) <- c("Phi", "% of Community")
rownames(mero.indicators) <- gsub("k__Bacteria;|p__|c__|o__|g__|f__|;g__;s__1||\\[|\\]", "", rownames(mero.indicators))
write.csv(mero.indicators, file = "C:/Users/Alex/Desktop/North_Temperate_Lakes-Microbial_Observatory/Data/meromictic_indicators.csv", row.names = T)
