...
png(file = paste(path2repo, "FigureS1_hypo_boxplot.png", sep = ""), width = 4, height = 3, units = "in", res = 144)
...
pairwise.wilcox.test(epi.data$epi.evenness, epi.data$epi.lakes, p.adjust.method = "bonferroni")
pairwise.wilcox.test(hypo.data$hypo.evenness, hypo.data$hypo.lakes, p.adjust.method = "bonferroni")
...
ggplot(data = plot.conn, aes(x = Lake, y = Connectivity)) + geom_boxplot() + scale_y_log10() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line.x = element_line(colour = "black"), axis.line.y = element_line(colour = "black"), axis.text = element_text(colour = "black"))
dev.off()

#Test significant differences
rank.test <- pairwise.wilcox.test(plot.conn$Connectivity, plot.conn$Lake, p.adjust.method = "bonferroni")
#print(rank.test)
...
```{r, annual_PCoAs, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r, rep_MAH_PCoA, echo=FALSE, warning=FALSE, message=FALSE}
